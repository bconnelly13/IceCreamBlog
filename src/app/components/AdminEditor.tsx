...
import { Post, TAG_OPTIONS } from '../data/mockData';
...
const RATING_KEYS: RatingKey[] = ['taste', 'vibe', 'location', 'value', 'presentation'];
...
const PRESET_FLAVORS = [
  ...
  'Rocky Road', 'Cookies & Cream', 'Salted Caramel', 'Pistachio',
];
...
export function AdminEditor({ initialPost, allTags, onPublish, onCancel }: AdminEditorProps) {
  ...
  const [ratings, setRatings] = useState<Record<RatingKey, number>>(
    initialPost?.ratings ?? { taste: 0, vibe: 0, location: 0, value: 0, presentation: 0 }
  );
  ...
  const flavorRows = showAllFlavors ? allPresetFlavors : allPresetFlavors.slice(0, COLLAPSED_COUNT);
  ...
  function validate() {
    ...
    if (Object.values(ratings).some(v => v === 0)) errs.push('All ratings must be filled');
    return errs;
  }
  ...
  function handlePublish() {
    ...
    setErrors([]);
    const post: Post = {
      id: initialPost?.id ?? String(Date.now()),
      title: title.trim(),
  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      ...
      <div className="px-4 pt-5 pb-4 flex items-center gap-3 border-b border-border">
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: '#8B6558', marginTop: 1 }}>
            {isEdit ? `Editing: ${initialPost.shopName}` : 'Share your latest ice cream adventure'}
          </p>
        </div>
      </div>
      ...
      <div className="px-4 py-5 space-y-6">
        {errors.length > 0 && (
          <div className="p-3 rounded-xl space-y-1" style={{ background: '#FDE8EF', border: '1px solid rgba(193,65,90,0.2)' }}>
            {errors.map(e => <p key={e} style={{ fontSize: 13, color: '#C1415A' }}>• {e}</p>)}
          </div>
        )}
        ...
        <Section title="The Basics">
          <Field label="Post Title">
            <TextInput value={title} onChange={setTitle} placeholder="e.g. Brambleberry Bliss at Jeni's" />
          </Field>
          <Field label="Shop Name">
            <TextInput value={shopName} onChange={setShopName} placeholder="e.g. Jeni's Splendid Ice Creams" />
          </Field>
          ...
          <Field label="Price per Scoop ($)">
            <TextInput value={price} onChange={setPrice} placeholder="6.50" type="number" />
          </Field>
        </Section>
        ...
        <Section title="Location">
          <Field label="Street Address">
            <TextInput value={address} onChange={setAddress} placeholder="714 N High St" />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="City">
              <TextInput value={city} onChange={setCity} placeholder="Columbus" />
            </Field>
            <Field label="State">
        ...
        <Section title="Flavors">
          ...
          >
            <div className="flex flex-wrap gap-2">
              {flavorRows.map(f => (
                ...
                >
                  {flavors.includes(f) && <span style={{ marginRight: 4 }}>✓</span>}
                  {f}
                </button>
              ))}
            </div>
          ...
          {hasMore && (
            ...
            >
              {showAllFlavors ? '↑ Show less' : `↓ Show all ${allPresetFlavors.length} flavors`}
            </button>
          )}
          ...
          {flavors.length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1">
              {flavors.map(f => (
                <span key={f} className="flex items-center gap-1 px-2.5 py-1 rounded-full text-sm"
                  style={{ background: '#FDE8EF', color: '#C1415A' }}>
                  {f}
        ...
        <Section title="Ratings">
          {RATING_KEYS.map(key => (
            <div key={key} className="flex items-center justify-between py-0.5">
              <label className="text-sm font-medium capitalize" style={{ color: '#1C0E0A' }}>{key}</label>
              <StarPicker value={ratings[key]} onChange={v => setRatings(prev => ({ ...prev, [key]: v }))} />
            </div>
          ))}
        </Section>
        ...
        <Section title="The Write-Up">
          <textarea
            ...
            style={{ background: '#FDF0E8', color: '#1C0E0A', fontFamily: 'var(--font-body)', lineHeight: 1.6 }}
          />
        </Section>
        ...
        <Section title="Hashtags">
          ...
          {/* Create new tag */}
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: '#8B6558' }}>
              Create a new hashtag
            </label>
            <div className="flex gap-2">
              <div className="relative flex-1">
                <Hash size={14} className="absolute left-3 top-1/2 -translate-y-1/2" color="#8B6558" />
                <input
                  value={newTag}
            ...
            {newCustomTags.length > 0 && (
              <p className="text-xs mt-1.5" style={{ color: '#8B6558' }}>
                New tags: {newCustomTags.map(t => `#${t}`).join(', ')} — will be added to the global list on publish
              </p>
            )}
          </div>
        </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-3">
      <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 600, color: '#1C0E0A' }}>{title}</h3>
      {children}
    </div>
  );
}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1">
      <label className="block text-xs font-semibold" style={{ color: '#8B6558' }}>{label}</label>
      {children}
    </div>
  );
}
