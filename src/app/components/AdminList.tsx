      ...
      <div className="px-4 pb-6 space-y-3">
        {posts.length === 0 && (
          <div className="flex flex-col items-center py-16 text-center gap-3">
            ...
            <p style={{ color: '#8B6558', fontSize: 15 }}>No posts yet. Create your first one!</p>
          </div>
        )}
        {posts.map(post => {
          ...
          return (
            ...
            >
              {/* Thumbnail */}
              <div className="shrink-0 rounded-xl overflow-hidden" style={{ width: 72, height: 72, background: '#F5EAE0' }}>
                {post.photos[0] ? (
                  <ImageWithFallback src={post.photos[0]} alt={post.title} className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                <p style={{ fontSize: 14, fontWeight: 600, color: '#1C0E0A', lineHeight: 1.3 }} className="truncate">
                  {post.title}
                </p>
                <p style={{ fontSize: 12, color: '#C1415A', fontWeight: 500, marginTop: 1 }} className="truncate">
                  {post.shopName}
                </p>
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  <span className="flex items-center gap-0.5" style={{ fontSize: 11, color: '#8B6558' }}>
                    <MapPin size={10} />{post.city}, {post.state}
                  </span>
                  <span style={{ fontSize: 11, color: '#8B6558' }}>{date}</span>
                  <span className="flex items-center gap-0.5" style={{ fontSize: 11, color: '#F59340', fontWeight: 600 }}>
                    <Star size={10} fill="#F59340" color="#F59340" />{avg.toFixed(1)}
                  </span>
                  <span style={{ fontSize: 11, color: '#8B6558' }}>
                    <DollarSign size={9} style={{ display: 'inline' }} />{post.price.toFixed(2)}
                  </span>
                </div>
              </div>
              ...
              <div className="shrink-0 flex items-center">
                <div className="p-2 rounded-xl" style={{ background: '#F5EAE0' }}>
